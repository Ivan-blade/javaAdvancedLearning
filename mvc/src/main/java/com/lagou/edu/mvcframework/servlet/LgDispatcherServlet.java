...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            // 如果包含controller开始注解扫描,创建白名单，如果为扫描到该注解，白名单为null，不生效，如果扫描到对value进行取值
            String[] whiteList = null;
            if(aClass.isAnnotationPresent(Security.class)) {
                Security annotation = aClass.getAnnotation(Security.class);
                if(annotation.value().length < 1) whiteList = annotation.value();
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                // 建立临时白名单用于赋值防止在遍历方法时直接修改类白名单的值
                // 如果方法中的白名单和类上同时出现白名单，方法上的白名单优先覆盖临时名单
                // 如果方法上没有出现白名单，则使用被类白名单赋值的临时白名单
                String[] tempList = whiteList;
                if(method.isAnnotationPresent(Security.class)) {
                    Security annotation = method.getAnnotation(Security.class);
                    if(annotation.value().length < 1) tempList = annotation.value();
                }
                // 如果标识，就处理
                ...
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url),tempList);
...
//        method.invoke() //
        ...
        // 创建username变量，根据赋值情况和最后的handler白名单情况判断是否执行方法
        ...
        // 判断不能通过的情况
        if(handler.getWhiteList() != null) {
            boolean permit = false;
            String[] whiteList = handler.getWhiteList();
            for(int i = 0; i < whiteList.length; i++) {
                if(whiteList[i].equals(username)) permit = true;
            }
            if (!permit) {
                resp.getWriter().write("403 forbidden permit not allow");
                return;
            }
        }
